(function() {
        ...
        if(data && data.movies) {
            ...
            receivedMovies.forEach(m => {
                ...
                if (!existsInLocalStorage) {
                    ...
                    // Формируем URL для поиска по названию и году (без использования Alloha API)
                    var url;
                    if (movieType === 'movie') {
                        console.log('Kinorium', 'Searching by movie title: ' + movieTitle + ', year: ' + movieYear);
                        ...
                        console.log('Kinorium', 'Searching by tv series title: ' + movieTitle + ', year: ' + movieYear);
                    ...
                    network.silent(url, function(data) {
                        if(data) {
                            ...
                            // Обрабатываем разные структуры ответа TMDB
                            ...
                            console.log('Kinorium', 'No data received from TMDB for: ' + movieTitle);
                        }
                        calculateProgress(receivedMoviesCount, processedItems++);
                    }, function(errorData) {
                        console.log('Kinorium', 'TMDB request error:', errorData);
                        calculateProgress(receivedMoviesCount, processedItems++);
                    });
                } else {
    ...
    function full(params, oncomplete, onerror) {
        // Возвращаем фильмы из хранилища и запускаем обновление в фоне
    ...
    function startPlugin() {
        ...
        // SETTINGS (упрощенные, без авторизации)
